...
class Block(nn.Module):
    def __init__(self, index, dim, num_head, norm_cfg=dict(type='BN', requires_grad=True), mlp_ratio=4.,
...
def DFormer_Tiny(pretrained=False, **kwargs):  # 81.5
    ...
    if pretrained:
        model.init_weights("/media/yuride/date/model/backbone/Dformer/DFormer_Tiny.pth.tar")
        print("loaded DFormer_Tiny")
...
def DFormer_Small(pretrained=False, **kwargs):  # 81.0
    model = DFormer(dims=[64, 128, 256, 512], mlp_ratios=[8, 8, 4, 4], depths=[2, 2, 4, 2], num_heads=[1, 2, 4, 8],
                    windows=[0, 7, 7, 7], **kwargs)
    if pretrained:
        model.init_weights("/media/yuride/date/model/backbone/Dformer/DFormer_Small.pth.tar")
        print("loaded DFormer_Small")
    return model
...
def DFormer_Base(pretrained=False, drop_path_rate=0.1, **kwargs):  # 82.1
    ...
    if pretrained:
        model.init_weights("/media/yuride/date/model/backbone/Dformer/DFormer_Base.pth.tar")
        print("loaded DFormer_Base")
    return model
...
def DFormer_Large(pretrained=False, drop_path_rate=0.1, **kwargs):  # 82.1
    ...
    if pretrained:
        model.init_weights("/media/yuride/date/model/backbone/Dformer/DFormer_Large.pth.tar")
        print("loaded DFormer_Base")
    return model

if __name__ == '__main__':
    input = torch.randn(2, 3, 224, 224).cuda()
    input_d = torch.randn(2, 1, 224, 224).cuda()
    net = DFormer_Tiny(pretrained=True).cuda()
    out = net(input, input_d)
    print('xx1', out[0].shape)
    print('xx2', out[1].shape)
    print('xx3', out[2].shape)
    print('xx4', out[3].shape)